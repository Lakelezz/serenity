...
impl Connection {
    pub async fn new(mut info: ConnectionInfo) -> Result<Connection> {
        ...
        let key = encryption_key(&mut stream).await?;
    ...
    async fn check_keepalive_timer(&mut self) -> Result<()> {
        if self.keepalive_timer.check() {
            ...
            self.client.send_json(&payload::build_heartbeat(nonce)).await?;
    ...
    pub async fn cycle(&mut self,
        ...
        let len = self.remove_unfinished_files(&mut sources, &opus_frame, &mut buffer,&mut mix_buffer).await?;
...
#[inline]
async fn encryption_key(client: &mut WsStream) -> Result<Key> {
    loop {
...
async fn start_ws_task(mut stream: SplitStream<WsStream>, tx: &Sender<ReceiverStatus>) -> Result<(Sender<i32>, JoinHandle<()>)> {
    ...
    let ws_task = tokio::spawn(async move {
            'outer: loop {
                // convert_ws_message(self.try_next().await.ok()
